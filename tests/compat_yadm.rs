...
/// Help/usage text embeds the invoked program's own name (`yadm` in the bash
/// reference, `ryadm` in this port), which otherwise makes every help-related
/// line a spurious diff. Collapse just that name, and only on lines that are
/// clearly part of the help/usage rendering (the `Usage:` line, the
/// two-space-indented command list, the `man` pointer, and the bootstrap
/// script-run banner) so unrelated compat strings like `yadm.io`,
/// `.config/yadm`, `--yadm-repo`, `YADM_*`, `yadm's`, or `yadm version` are
/// left untouched.
fn mask_help_impl_name(l: &mut String) {
    let is_help_line = l.starts_with("Usage: yadm")
        || l.starts_with("Usage: ryadm")
        || l.starts_with("  yadm ")
        || l.starts_with("  ryadm ")
        || l.starts_with("Use \"man yadm\"")
        || l.starts_with("Use \"man ryadm\"")
        || l.contains("Script run via: yadm bootstrap")
        || l.contains("Script run via: ryadm bootstrap");
    if is_help_line {
        // Handle the `man yadm"`/`man ryadm"` pointer first: the name is
        // followed by a quote, not a space, so the space-suffixed replacements
        // below would miss it.
        *l = l
            .replace("man ryadm\"", "man <impl>\"")
            .replace("man yadm\"", "man <impl>\"")
            .replace("ryadm ", "<impl> ")
            .replace("yadm ", "<impl> ");
    }
}
